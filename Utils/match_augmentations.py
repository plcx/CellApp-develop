...
def contour_distance_for_nuc_and_mem(mem_label,nuc_label, mem_edt_threshold=9,nuc_edt_threshold=9):
    '''
    :param label:  Binary label of the target
    :param center_stack:
    :return:
    '''
    # ---------------------mem contour generation part------------------------------------------------------------------
    background_edt = distance_transform_edt(mem_label == 0) # calculate the distance from backgroud to labelmembrane
    # near -> far, small->large
    # print(np.unique(background_edt.astype(int), return_counts=True))

    background_edt[background_edt > mem_edt_threshold] = mem_edt_threshold
    norm_mem_edt = ((mem_edt_threshold - background_edt) + nuc_edt_threshold)/ (mem_edt_threshold +nuc_edt_threshold)
    # the normalize distance from labelmembrane to backgroud
    # --------------------------------------------------------------------------------------------------------------

    # ---------------------seg contour generation part------------------------------------------------------------------
    background_edt = distance_transform_edt(nuc_label == 0)  # calculate the distance from backgroud to labelmembrane
    # near -> far, small->large
    background_edt[background_edt > nuc_edt_threshold] = nuc_edt_threshold
    norm_nuc_edt = (nuc_edt_threshold - background_edt)  / (mem_edt_threshold + nuc_edt_threshold)
    # the normalize distance from  to backgroud
    # --------------------------------------------------------------------------------------------------------------
    return (norm_mem_edt+norm_nuc_edt).astype(np.float32)
