...
from Utils.gui_data_io import check_folder


def save_the_morphological_and_nuc_location_file(name_dictionary_path, embryo_name, running_max_time,cd_file_path,stat_file_root,raw_z_resolution=92,z_resolution=214,three_d_resolution=0.18):
    ...
    # ---------------------read cd file---------------------------------
    # cd_file_path = os.path.join(CD_file_root, )
    # cd_file_all_dict = {}
    ...
    for tp in tqdm(range(1, running_max_time + 1), desc='generating {} nucloc file per time point'.format(embryo_name)):
        ...
        with open(os.path.join(stat_path_this_embryo, '{}_{}_segCell_volume.txt'.format(embryo_name, str(tp).zfill(3))),
                  'rb') as handle:
            volume_dict = pickle.load(handle)
        # read the cell surface area pkl file
        with open(os.path.join(stat_path_this_embryo, '{}_{}_segCell_surface.txt'.format(embryo_name, str(tp).zfill(3))),
                  'rb') as handle:
            surface_dict = pickle.load(handle)
        # x y z is the size of the 3D volume, designed by you
        df_nucLoc = pd.DataFrame(
            columns=['nucleus_label', 'nucleus_name', 'x_256', 'y_256', 'z_{}'.format(z_resolution), 'volume', 'surface', 'note'])
