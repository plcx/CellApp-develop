...
from Utils.label_utils import check_folder
from Utils.label_data_structure import read_cd_file
from Utils.label_data_io import nib_save
import os
import numpy as np
...
def combine_slices(config):
    ...
    for idx_embryo,embryo_name in enumerate(embryo_names):
        ...
        if is_save_seg_cell_with_cd_file:
            ...
            # labeling as sphere
            for idx, _ in enumerate(tqdm(mpPool.imap_unordered(save_nuc_point, configs_sphere), total=len(configs_sphere),
...
def save_nuc_point(para):
    ...
    for cell_name,cell_info in nuc_dict.items():
        ...
        cell_radius = int(cell_info[3]/6)#/8 small nuc. /4 normal nuc
        label = labels[list(nuc_dict.keys()).index(cell_name)]
...
if __name__ == "__main__":
    ...
    config = dict(num_slice=92,
                  ...
                  embryo_names=['191108plc1p1', '200113plc1p3'],
                  #max_times = [240, 240, 240, 90, 205, 255],# length of embryo
                  max_times = [205, 195],# length of embryo
                  xy_resolution = 0.09,
                  z_resolution = 0.42,
                  out_size = [256, 356, 214], # todo: need to be calculated with the vertical image amount
                  raw_folder=r'  ',
                  target_folder=r"E:\NucleuLabeling\fulldataset\testing\SegNuc",
                  save_nuc = False,
                  save_memb = False,
                  lineage_file = True,
                  cd_file_path = r'E:\NucleuLabeling\fulldataset\testing\CDfiles',
