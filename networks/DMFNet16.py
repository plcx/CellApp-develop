...
class MFUnit(nn.Module):
    def __init__(self, num_in, num_out, stride=1, groups=1, norm=None):
        ...
        self.conv1x1x1_in2 = Conv3dBlock(num_in=num_out//4, num_out=num_mid,kernel_size=1, stride=1, norm=norm) # multiplexer
...
class DMFUnit(nn.Module):
    ...
    def forward(self, x):
        ...
        shortcut = x
        if hasattr(self, "conv1x1x1_shortcut"):
            # print('conv1x1x1_shortcut',x.shape, shortcut.shape)
            shortcut = self.conv1x1x1_shortcut(shortcut)
        elif hasattr(self, "conv2x2x2_shortcut"):
            # print('conv2x2x2_shortcut',x.shape, shortcut.shape)
...
class DMFNet(nn.Module):
    def __init__(self, in_channels=1, n_first=32, conv_channels=128, groups=8, norm="bn", out_class=2):
        ...
        # https://arxiv.org/pdf/1904.03355.pdf
...
class MDFNetBin(DMFNet):
    ...
    #def forward(self, memb, nuc):todo
    def forward(self, memb):
        ...
        #  encoder -- memb
        x0_nuc = self.first_conv_nuc(memb) # todo , no nuc used
...
class TopoDMFNet(DMFNet):
    def __init__(self, in_channels=1, n_first=32, conv_channels=128, groups=8, norm="bn", out_class=2):
        ...
        # ===============================================================
        #    decoder -- restore constrainst
        # ===============================================================
        self.upsample1_topo = nn.Upsample(scale_factor=4, mode="trilinear", align_corners=False)
        self.decoder_block1_topo = MFUnit(2 * conv_channels, conv_channels, groups=groups, stride=1,
                                     norm=norm)

        self.upsample2_topo = nn.Upsample(scale_factor=4, mode="trilinear", align_corners=False)
        self.decoder_block2_topo = MFUnit(conv_channels, n_first, groups=groups, stride=1,
                                     norm=norm)

        # self.upsample3_topo = nn.Upsample(scale_factor=2, mode="trilinear", align_corners=False)
        # self.decoder_block3_topo = MFUnit(conv_channels + n_first, n_first, groups=groups, stride=1, norm=norm)
        # self.upsample4_topo = nn.Upsample(scale_factor=2, mode="trilinear", align_corners=False)

        # num_in, num_out, kernel_size=1, stride=1, padding=None, groups=1, norm=Non
        self.out_conv_topo = Conv3dBlock(n_first, out_class, kernel_size=1, stride=1, norm=norm)

        self.topo_out=nn.Sigmoid()
    ...
    def forward(self, memb):
        ...
        #  decoder for segmentation
        y1_topo = self.upsample1_topo(x3)
        # y1_topo = torch.cat([x2, y1_topo], dim=1)
        y1_topo = self.decoder_block1_topo(y1_topo)
        y2_topo = self.upsample2_topo(y1_topo)
        # y2_topo = torch.cat([x1, y2_topo], dim=1)
        y2_topo = self.decoder_block2_topo(y2_topo)
        # y3_topo = self.upsample3_topo(y2_topo)
        # y3_topo = torch.cat([x0, y3_topo], dim=1)
        # y3_topo = self.decoder_block3_topo(y3_topo)
        # y4_topo = self.upsample4_topo(y3_topo)
        y4_topo = self.out_conv_topo(y2_topo)
        out2 = self.topo_out(y4_topo)

        return out1, out2

class TopoConstraintDMFNet(DMFNet):
    def __init__(self, in_channels=1, n_first=32, conv_channels=128, groups=8, norm="bn", out_class=2):
        super().__init__(in_channels, n_first, conv_channels, groups, norm, out_class)
        self.first_conv_nuc = copy.deepcopy(self.first_conv)
        self.encoder_block1_nuc = copy.deepcopy(self.encoder_block1)
        ...
        # ===============================================================
        self.upsample1_topo = nn.Upsample(scale_factor=4, mode="trilinear", align_corners=False)
        self.decoder_block1_topo = MFUnit(2 * conv_channels, n_first, groups=groups, stride=1,
                                     norm=norm)

        self.upsample2_topo = nn.Upsample(scale_factor=4, mode="trilinear", align_corners=False)
        # self.decoder_block2_topo = MFUnit(conv_channels , conv_channels, groups=groups, stride=1,norm=norm)

        # self.upsample3_topo = nn.Upsample(scale_factor=2, mode="trilinear", align_corners=False)
        # self.decoder_block2_topo = MFUnit(conv_channels , n_first, groups=groups, stride=1, norm=norm)

        # self.upsample3_topo = nn.Upsample(scale_factor=4, mode="trilinear", align_corners=False)
        # num_in, num_out, kernel_size=1, stride=1, padding=None, groups=1, norm=Non
        self.out_conv_topo = Conv3dBlock(n_first, out_class, kernel_size=1, stride=1, norm=norm)
    ...
    def forward(self, memb):
        ...
        #  decoder for segmentation
        y1_topo = self.upsample1_topo(x3) # dimension *2
        # y1_topo = torch.cat([x2, y1_topo], dim=1)
        y1_topo = self.decoder_block1_topo(y1_topo) # ouput channel / 2 = conv_channels
        y2_topo = self.upsample2_topo(y1_topo) # dimension * 4
        # y2_topo = torch.cat([x1, y2_topo], dim=1)
        # y2_topo = self.decoder_block2_topo(y2_topo)  # ouput channel / 2 = n_first
        # y3_topo = self.upsample3_topo(y2_topo) # dimension * 2
        # y3_topo = torch.cat([x0, y3_topo], dim=1)
        # y3_topo = self.decoder_block3_topo(y3_topo)
        # y4_topo = self.upsample4_topo(y3_topo)
        y4_topo = self.out_conv_topo(y2_topo)
