#  Resource: https://github.com/mattmacy/vnet.pytorch
...
class InputTransition(nn.Module):
    def __init__(self, input_channel, out_channel,elu):
        super(InputTransition, self).__init__()
        self.conv1 = nn.Conv3d(input_channel, out_channel, kernel_size=5, padding=2)
    ...
    def forward(self, x):
        ...
        out = self.bn1(out_tmp)
        # split input in to 16 channels
        # careful here!
        x16 = torch.cat((x, x, x, x, x, x, x, x,
                         # x, x, x, x, x, x, x, x
                         ), 1)  # input channel should match the output size
...
##===============================================================
##             contraction translation layers
##===============================================================
...
class UpTransition(nn.Module):
    def __init__(self, inChans, outChans, nConvs, elu, dropout=False):
        ...
        self.relu2 = ELUCons(elu, outChans)
        if dropout:
            self.do1 = nn.Dropout3d()  # determine whether to add 3D dropout
        self.ops = _make_nConv(outChans, nConvs, elu)
...
class OutputTransition(nn.Module):
    def __init__(self, inChans, elu, nll):
        ...
        if nll:
            ...
            self.softmax = F.softmax # default that is false
...
class VNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out16 = self.in_tr(x)
        # print(out16.shape)
        out32 = self.down_tr32(out16)  # out 32 channel
        # print(out32.shape)
...
#     net = VNet()
#     print(net)
#     input = torch.randn(8, 1, 128, 128, 128)
#     output = net(input)
#     print(output.shape)
